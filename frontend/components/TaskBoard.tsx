  ...
  useEffect(() => {
    ...
    socket.on('task.statusChanged', (payload: { id: string; status: TaskStatus }) => {
      qc.setQueryData<Task[]>(['tasks'], (old = []) =>
        old.map((t) => (t.id === payload.id ? { ...t, status: payload.status } : t)),
      );
    });
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-5">
        {COLUMNS.map((col) => {
          ...
          return (
            <div key={col.status} className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
              <div className="mb-4 flex items-center justify-between">
                ...
                <span className="text-sm font-medium text-gray-400">{colTasks.length}</span>
              </div>
              ...
              <div className="space-y-3">
                ...
                {colTasks.length === 0 && (
                  <p className="py-6 text-center text-xs text-gray-400">Нет задач</p>
                )}
              </div>
            </div>
          );
